...
function ProductCard({ icon: Icon, name, price, store, rating, gradient }: Product) {
  return (
    <div className="w-44 rounded-2xl bg-[var(--bone-2)]/95 p-2.5 shadow-2xl shadow-black/40 ring-1 ring-black/5 backdrop-blur-sm">
      ...
      <p className="truncate text-[13px] font-semibold leading-tight text-[var(--ink)]">{name}</p>
      <div className="mt-1 flex items-center justify-between">
        <span className="text-[13px] font-bold text-[var(--ink)]">{price}</span>
      ...
      <p className="mt-0.5 truncate text-[10px] text-[var(--ink-soft)]">{store}</p>
    </div>
  );
}
...
const CARDS: Array<{
  ...
  {
    product: { icon: Headphones, name: 'Audífonos Pro', price: '$189.000', store: 'TecnoStore', rating: '4.8', gradient: 'from-[#c75a2b] to-[#a8431d]' },
    className: 'right-[5%] top-[9%]',
    depth: { scale: 1, opacity: 1 },
    float: { y: [0, -16, 0], rotate: [-3, 1, -3], duration: 6.5, delay: 0 },
  },
  {
    product: { icon: Watch, name: 'Smartwatch X', price: '$320.000', store: 'RelojesCO', rating: '4.9', gradient: 'from-[#2f5d4f] to-[#244a3f]' },
    className: 'right-[30%] top-[30%]',
    depth: { scale: 0.9, opacity: 0.9, blur: 'blur-[0.5px]' },
    float: { y: [0, 14, 0], rotate: [2, -2, 2], duration: 8, delay: 0.6 },
  },
  {
    product: { icon: Camera, name: 'Cámara 4K', price: '$890.000', store: 'FotoPro', rating: '5.0', gradient: 'from-[#d97a4f] to-[#c75a2b]' },
    className: 'right-[3%] top-[50%]',
    depth: { scale: 1, opacity: 1 },
    float: { y: [0, -13, 0], rotate: [3, -1, 3], duration: 7.2, delay: 1.1 },
  },
  {
    product: { icon: Shirt, name: 'Camiseta Premium', price: '$79.000', store: 'ModaUrbana', rating: '4.7', gradient: 'from-[#3a7060] to-[#2f5d4f]' },
    className: 'right-[26%] top-[70%]',
    depth: { scale: 0.82, opacity: 0.7, blur: 'blur-[1px]' },
    float: { y: [0, 18, 0], rotate: [-2, 2, -2], duration: 9, delay: 0.3 },
  },
  {
    product: { icon: Smartphone, name: 'Smartphone 5G', price: '$1.450.000', store: 'CelularesCol', rating: '4.9', gradient: 'from-[#b06a4a] to-[#8a4a2f]' },
  ...
  return (
    <div className="absolute inset-0 overflow-hidden pointer-events-none select-none" aria-hidden="true">
      ...
      <div
        ...
        style={{ background: 'radial-gradient(circle at 70% 30%, rgba(199,90,43,0.18), transparent 60%)' }}
      />
